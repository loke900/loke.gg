...
"@
    ...
    # Start hotkey monitoring in a separate runspace
    $script:clickerRunspace = [RunspaceFactory]::CreateRunspace()
    $script:clickerRunspace.ApartmentState = "STA"
    $script:clickerRunspace.ThreadOptions = "ReuseThread"
    $script:clickerRunspace.Open()
    
    $script:clickerPipeline = $script:clickerRunspace.CreatePipeline()
...
"@
        ...
        $leftCpsSlider.Add_Scroll({
            $leftCps = $leftCpsSlider.Value
            $leftCpsValue.Text = $leftCps.ToString()
        })
        ...
        $rightCpsSlider.Add_Scroll({
            $rightCps = $rightCpsSlider.Value
            $rightCpsValue.Text = $rightCps.ToString()
        })
        ...
        $leftRandSlider.Add_Scroll({
            $leftRandomization = $leftRandSlider.Value
            $leftRandValue.Text = "$leftRandomization%"
        })
        ...
        $rightRandSlider.Add_Scroll({
            $rightRandomization = $rightRandSlider.Value
            $rightRandValue.Text = "$rightRandomization%"
        })
        ...
        $titleBar.Add_MouseMove({
            ...
            if ($isDragging) {
                $form.Location = New-Object System.Drawing.Point(
                    ($form.Location.X + $e.X - $dragStart.X),
                    ($form.Location.Y + $e.Y - $dragStart.Y)
                )
            }
        })
        ...
        $clickTimer.Add_Tick({
            ...
            # Left Click
            if ($leftClickEnabled -and [KeyboardHook]::GetAsyncKeyState([KeyboardHook]::VK_LBUTTON) -band 0x8000) {
                $randomizationFactor = $leftRandomization / 100.0
                $randomMultiplier = 1.0 + (($random.NextDouble() * 2 - 1) * $randomizationFactor)
                $currentCps = [Math]::Max(1, $leftCps * $randomMultiplier)
                $requiredDelay = 1000.0 / $currentCps
                
                if (($now - $leftLastClick).TotalMilliseconds -ge $requiredDelay) {
            ...
            # Right Click
            if ($rightClickEnabled -and [KeyboardHook]::GetAsyncKeyState([KeyboardHook]::VK_RBUTTON) -band 0x8000) {
                $randomizationFactor = $rightRandomization / 100.0
                $randomMultiplier = 1.0 + (($random.NextDouble() * 2 - 1) * $randomizationFactor)
                $currentCps = [Math]::Max(1, $rightCps * $randomMultiplier)
                $requiredDelay = 1000.0 / $currentCps
                
                if (($now - $rightLastClick).TotalMilliseconds -ge $requiredDelay) {
        ...
        $hotkeyTimer.Add_Tick({
            # Left Hotkey
            $leftKeyDown = ([KeyboardHook]::GetAsyncKeyState($leftHotkeyVK) -band 0x8000) -ne 0
            if ($leftKeyDown -and -not $lastLeftHotkeyDown) {
                $leftClickEnabled = -not $leftClickEnabled
                $leftStatusIndicator.BackColor = if ($leftClickEnabled) { 
                    [System.Drawing.Color]::FromArgb(0, 200, 0) 
                } else { 
                    [System.Drawing.Color]::FromArgb(200, 0, 0) 
                }
                $leftStatusLabel.ForeColor = if ($leftClickEnabled) { 
                    [System.Drawing.Color]::FromArgb(0, 200, 0) 
                } else { 
                    [System.Drawing.Color]::FromArgb(200, 0, 0) 
                }
                $leftStatusLabel.Text = if ($leftClickEnabled) { 
                    "ENABLED ($leftHotkeyName)" 
                } else { 
                    "DISABLED ($leftHotkeyName)" 
                }
            }
            $lastLeftHotkeyDown = $leftKeyDown
            
            # Right Hotkey
            $rightKeyDown = ([KeyboardHook]::GetAsyncKeyState($rightHotkeyVK) -band 0x8000) -ne 0
            if ($rightKeyDown -and -not $lastRightHotkeyDown) {
                $rightClickEnabled = -not $rightClickEnabled
                $rightStatusIndicator.BackColor = if ($rightClickEnabled) { 
                    [System.Drawing.Color]::FromArgb(0, 200, 0) 
                } else { 
                    [System.Drawing.Color]::FromArgb(200, 0, 0) 
                }
                $rightStatusLabel.ForeColor = if ($rightClickEnabled) { 
                    [System.Drawing.Color]::FromArgb(0, 200, 0) 
                } else { 
                    [System.Drawing.Color]::FromArgb(200, 0, 0) 
                }
                $rightStatusLabel.Text = if ($rightClickEnabled) { 
                    "ENABLED ($rightHotkeyName)" 
                } else { 
                    "DISABLED ($rightHotkeyName)" 
                }
            }
            $lastRightHotkeyDown = $rightKeyDown
        })
        $hotkeyTimer.Start()
    ...
    # Add the script to the pipeline and execute asynchronously
    $script:clickerPipeline.Commands.AddScript($clickerScript)
    $script:clickerPipeline.Commands.Add("Out-String")
    $script:clickerPipeline.InvokeAsync()
}

# Initialize the autoclicker in background
...
# Start a background job to monitor ALT+C hotkey
...
"@
    ...
    while ($true) {
        $altPressed = ([HotkeyMonitor]::GetAsyncKeyState([HotkeyMonitor]::VK_MENU) -band 0x8000) -ne 0
        $cPressed = ([HotkeyMonitor]::GetAsyncKeyState([HotkeyMonitor]::VK_C) -band 0x8000) -ne 0
        
        if ($altPressed -and $cPressed) {
            # Signal that ALT+C was pressed
            $true
...
# Check for ALT+C press
while ($true) {
    if ((Receive-Job $hotkeyJob -ErrorAction SilentlyContinue) -contains $true) {
        Write-Host -ForegroundColor Green "ALT+C detected - launching autoclicker..."
        
        # Launch autoclicker in a separate process
        $clickerScript = {
            Add-Type -AssemblyName System.Windows.Forms
            Add-Type -AssemblyName System.Drawing
            Add-Type -TypeDefinition @"
...
"@
            ...
            $titleBarLabel.Add_MouseMove({
                ...
                if ($isDragging) {
                    $form.Location = New-Object System.Drawing.Point(
                        ($form.Location.X + $e.X - $dragStart.X + $titleBarLabel.Location.X),
                        ($form.Location.Y + $e.Y - $dragStart.Y)
                    )
                }
            })
            ...
            $clickTimer.Add_Tick({
                ...
                # Left Click
                if ($leftClickEnabled -and [MouseHelper]::GetAsyncKeyState([MouseHelper]::VK_LBUTTON) -band 0x8000) {
                    $randomizationFactor = $leftRandomization / 100.0
                    $randomMultiplier = 1.0 + (($random.NextDouble() * 2 - 1) * $randomizationFactor)
                    $currentCps = [Math]::Max(1, $leftCps * $randomMultiplier)
                    $requiredDelay = 1000.0 / $currentCps
                ...
                # Right Click
                if ($rightClickEnabled -and [MouseHelper]::GetAsyncKeyState([MouseHelper]::VK_RBUTTON) -band 0x8000) {
                    $randomizationFactor = $rightRandomization / 100.0
                    $randomMultiplier = 1.0 + (($random.NextDouble() * 2 - 1) * $randomizationFactor)
                    $currentCps = [Math]::Max(1, $rightCps * $randomMultiplier)
                    $requiredDelay = 1000.0 / $currentCps
            ...
            $hotkeyTimer.Add_Tick({
                # Left Hotkey
                $leftKeyDown = ([MouseHelper]::GetAsyncKeyState($leftHotkeyVK) -band 0x8000) -ne 0
                if ($leftKeyDown -and -not $lastLeftHotkeyDown) {
                    $leftClickEnabled = -not $leftClickEnabled
                    $leftStatusIndicator.BackColor = if ($leftClickEnabled) { 
                        [System.Drawing.Color]::FromArgb(0, 200, 0) 
                ...
                # Right Hotkey
                $rightKeyDown = ([MouseHelper]::GetAsyncKeyState($rightHotkeyVK) -band 0x8000) -ne 0
                if ($rightKeyDown -and -not $lastRightHotkeyDown) {
                    $rightClickEnabled = -not $rightClickEnabled
                    $rightStatusIndicator.BackColor = if ($rightClickEnabled) { 
                        [System.Drawing.Color]::FromArgb(0, 200, 0) 
        ...
        # Start the clicker in a separate runspace
        $clickerRunspace = [RunspaceFactory]::CreateRunspace()
        $clickerRunspace.ApartmentState = "STA"
        $clickerRunspace.ThreadOptions = "ReuseThread"
        $clickerRunspace.Open()
        
        $clickerPipeline = $clickerRunspace.CreatePipeline()
        $clickerPipeline.Commands.AddScript($clickerScript)
        $clickerPipeline.Commands.Add("Out-String")
        $clickerPipeline.Invoke()
        
        Start-Sleep -Milliseconds 100  # Let the UI start
    }
    
    # Continue with BAM script
    break
}

# Original BAM processing continues
