...
function Launch-Autoclicker {
    # Create the autoclicker GUI
...
Write-Host "";
Write-Host -ForegroundColor Yellow "   Note: Press Enter on 'Clicker' option twice to launch autoclicker"
Write-Host "";
...
# Create a custom object for the clicker option
$clickerOption = [PSCustomObject]@{
    'Examiner Time' = " "
    'Last Execution Time (UTC)' = " "
    'Last Execution User Time' = " "
    Application = ">>> CLICKER <<<"
    Path = "Double-click this option to launch autoclicker"
    Signature = " "
    User = " "
    SID = " "
    Regpath = " "
...
# Add the clicker option to the BAM results
$BamWithClicker = @($clickerOption) + @($Bam)

# Track if clicker has been selected
$clickerSelectedCount = 0
$clickerLaunched = $false

# Function to handle Out-GridView with clicker detection
function Show-ClickerGridView {
    param($Data, $Title)
    
    # Convert to DataTable for better handling
    ...
    $dataGridView.Add_CellDoubleClick({
        ...
        $app = $row.Cells["Application"].Value
        
        if ($app -eq ">>> CLICKER <<<") {
            $script:clickerSelectedCount++
            
            if ($script:clickerSelectedCount -eq 2 -and -not $script:clickerLaunched) {
                $script:clickerLaunched = $true
                $form.Close()
                
                # Launch autoclicker in a separate runspace
                ...
                Write-Host -ForegroundColor Green "Autoclicker launched! Close the clicker window to return to BAM results."
            }
        }
    })
...
# Show the grid view with clicker option
Show-ClickerGridView -Data $BamWithClicker -Title "BAM key entries $($Bam.count)  - User TimeZone: ($UserTime) -> ActiveBias: ( $Bias) - DayLightTime: ($Day) - [Double-click '>>> CLICKER <<<' to launch autoclicker]"
